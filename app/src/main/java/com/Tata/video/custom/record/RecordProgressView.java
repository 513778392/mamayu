package com.Tata.video.custom.record;
...
public class RecordProgressView extends View {
    ...
    private void init() {
        ...
        mRecordColor = 0xffea377f;
        mPendingColor = 0xffea377f;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (ClipInfo clipInfo : mClipInfoList) {
            ...
            switch (clipInfo.clipType) {
                case ClipInfo.CLIP_TYPE_SPACE:
                    canvas.drawRect(totalWidth - DpUtil.dp2px(1), 0f, newWidth, getHeight(), mSpacePaint);
                    break;
        ...
        if (lastTotalProgress + mCurClipInfo.progress < mMinDuration) {
            canvas.drawRect(mMinDuration / (float) mMaxDuration * getWidth(), 0f,
                    mMinDuration / (float) mMaxDuration * getWidth() + DpUtil.dp2px(2), getHeight(), mSpacePaint);
        }
        if (isCursorShow || isInProgress) {
            canvas.drawRect(totalWidth, 0f, totalWidth + DpUtil.dp2px(2), getHeight(), mSpacePaint);
        }
    }
